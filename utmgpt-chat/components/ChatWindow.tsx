...
function ThinkingIndicator(props: { aiEmoji?: string }) {
  return (
    <div className='flex max-w-[80%] mb-8 mr-auto'>
      <SidebarTrigger />
      <div className='mr-4 border bg-secondary -mt-2 rounded-full w-10 h-10 flex-shrink-0 flex items-center justify-center'>{props.aiEmoji}</div>
    ...
    <div className='flex flex-col max-w-[768px] mx-auto pb-12 w-full'>
      {props.messages.map((m, i) => {
        ...
        if (m.role === 'assistant') {
          ...
          const assistantMessageCount = props.messages.slice(0, i + 1).filter((msg) => msg.role === 'assistant').length;
          ...
          console.log(`🔍 Looking for sources for assistant message ${i} (count: ${assistantMessageCount}):`);
          console.log('  - Available keys:', Object.keys(props.sourcesForMessages));
        ...
        return <ChatMessageBubble key={m.id} message={m} aiEmoji={props.aiEmoji} sources={sources} />;
      })}
      {props.hasUserMessageWaitingForResponse && <ThinkingIndicator aiEmoji={props.aiEmoji} />}
    </div>
  );
}
...
function StickyToBottomContent(props: { content: ReactNode; footer?: ReactNode; className?: string; contentClassName?: string }) {
  ...
  return (
    <div ref={context.scrollRef} style={{ width: '100%', height: '100%' }} className={cn('grid grid-rows-[1fr,auto]', props.className)}>
...
export function ChatWindow(props: {
  ...
  const hasUserMessageWaitingForResponse =
    chat.messages.length > 0 && chat.messages[chat.messages.length - 1]?.role === 'user' && (chat.isLoading || intermediateStepsLoading);
